    ...
    async def callback(self, device, advertisement_data):
        ...
        if advertisement_data.manufacturer_data:
            for _, v in advertisement_data.manufacturer_data.items():
                try:
                    payload = v.decode("utf-8", errors="ignore")
                    ...
                    # Adaptive path-loss exponent (n) heuristic
                    ...
                    # Save the devices_seen dictionary to a JSON file
                    with open("ble_devices.json", "w") as f:
                        json.dump(self.devices_seen, f, indent=2)

                    # Print device information
                    print(f"🛰 {device.address} | RSSI: {raw_rssi} → {smoothed_rssi:.2f} | Distance: {distance}m | Payload: {payload}")
